...
// Try to load canvas, fallback to text if not available
let canvas;
try {
    canvas = require('canvas');
} catch (error) {
    console.warn('⚠️ Canvas nicht installiert. Fallback auf Text-Karten. Installiere Canvas für PNG-Unterstützung: npm install canvas');
    canvas = null;
}
...
class BingoPNGGenerator {
    ...
    /**
     * Generate text fallback when Canvas is not available
     */
    generateTextFallback(card, options = {}) {
        ...
        for (let row = 0; row < size; row++) {
            ...
            for (let col = 0; col < size; col++) {
                ...
                rowText += `${statusIcon} ${position}: ${cellText.substring(0, 20)}${cellText.length > 20 ? '...' : ''}\n`;
            }
            text += rowText + '\n';
        }
    ...
    async generateQuickUpdatePNG(card, options = {}) {
        ...
        return this.generateCardPNG(card, {
            ...options,
            width: 800,
            height: 900
        });
